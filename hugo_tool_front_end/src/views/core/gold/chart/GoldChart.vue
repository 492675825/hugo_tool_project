<template>
  <div>
    ...
    <el-row :gutter="10">
      <el-col :span="24">
        <el-card shadow="always">
          <div id="line" style="width:100%;height: 300px; margin-bottom: 40px"/>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="10">
      <el-col :span="24">
        <el-card shadow="always">
          <div id="bar" style="width:100%;height: 300px;margin-bottom: 40px"/>
        </el-card>
      </el-col>
    </el-row>
  </div>
...
export default {
  ...
  methods: {
    ...
    card_data() {
      axios({
        url: this.baseLink.baseURL + '/chart/card',
        method: 'get'
      }).then(res => {
        this.card_data_list = res.data
        this.version_date = this.card_data_list[0].version_date
        this.close_price = parseFloat(this.card_data_list[0].close)
        this.up_down = parseFloat(this.card_data_list[0].up_or_down)
    ...
    line_data() {
      axios({
        url: this.baseLink.baseURL + '/chart/line',
        method: 'get'
      }).then(res => {
        this.line_data_list = res.data
        const version_date_list = []
        for (let i = 0; i < this.line_data_list.length; i = i + 1) {
          version_date_list.push(this.line_data_list[i].month_number)
        }
        this.line_version_date = version_date_list
        const current_sum_mapping_list = []
        for (let i = 0; i < this.line_data_list.length; i++) {
          current_sum_mapping_list.push(this.line_data_list[i].current_sum_mapping)
        }
        this.line_current_mapping = current_sum_mapping_list
        const his_avg_sum_mapping_list = []
        for (let i = 0; i < this.line_data_list.length; i++) {
          his_avg_sum_mapping_list.push(this.line_data_list[i].history_average_mapping)
        }
    ...
    bar_data() {
      axios({
        url: this.baseLink.baseURL + '/chart/bar',
        method: 'get'
      }).then(res => {
        const bar_list = res.data
        // console.log(bar_list)
        for (let i = 0; i < bar_list.length; i++) {
          this.bar_year_number.push(bar_list[i].year_number)
        }
        for (let i = 0; i < bar_list.length; i = i + 1) {
          this.bar_max_earning_rate.push(bar_list[i].max_earning_rate) // toFixed保留小数点后几位
        }
        for (let i = 0; i < bar_list.length; i = i + 1) {
          this.bat_average_earning_rate.push(bar_list[i].average_earning_rate)
        }
